<template>
  <transition name="fade-y" appear>
    ...
    >
      ...
      <div
        class="w-full xl:w-10/12 2xl:w-8/12 2xl:px-12 sm:px-8 xl:px-0 flex flex-col xl:flex-row justify-start items-center xl:justify-between xl:items-start xl:space-x-10 relative"
      >
        <loader v-if="loading" style="height:100px" class="relative order-2 z-10 xl:order-1" />
        <div
          v-else
        ...
        >
          <transition-group name="fade-y" mode="in-out" appear>
            <post-card
              v-for="p of posts"
              :key="p.pid"
          ...
          <loader v-if="!loading && extraPosts" class="pb-4 pt-4" />

          <div v-else-if="!loading && !extraPosts" class="h-20 pt-4">
            <i
