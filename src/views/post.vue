    ...
    <transition v-else name="fade-y" appear>
      ...
      <div class="flex justify-start items-start flex-col space-y-20 w-full h-full transition duration-300">
        <div :class="{ 'translate-y-5': viewingImg }" class="w-full h-auto relative transform transition duration-300">
          ...
          >
            <router-link :to="'/profile/' + postOwner.uid" tag="div" class="transform -translate-y-2/3 w-full sm:w-10/12 sm:max-w-md">
              <user-card
            ...
            <div class=" bg-gray-100 h-10 transform -translate-y-2/3 shadow-xl rounded-xl flex justify-center items-center px-5 font-semibold">
              Hosting date:
              {{ displayedDate ? displayedDate.date + ' at ' + displayedDate.time : 'xx/xx/xxxx at xx:xx' }}
            </div>
          </div>
        </div>
        ...
        <div class="px-10 py-5 bg-gray-100 w-full rounded-t-6xl shadow-3xl relative pb-20 flex-grow">
          <div class="top-0 absolute right-20 flex justify-end items-center transform -translate-y-1/2 gap-x-4 z-50 w-full">
            <!-- title and date input and commit button-->
            <div :class="{ hidden: !inEditingMode }" class="w-2/12 flex items-center justify-start relative ml-32">
              <base-input name="Title" :lazy="200" size="0" :modelValue="title" @update:modelValue="handleTitle" />
            </div>
            <div :class="{ hidden: !inEditingMode }" class="relative">
              <div class="absolute top-0 z-50 transform -translate-y-1/2 bg-gray-50 left-4 font-semibold text-sm rounded-full px-px">
                Hosting date
              </div>
              <datepicker class="outline-none shadow-2xl focus:outline-none rounded-2xl font-semibold z-0 bg-gray-50 w-48" v-model="pickedDate" />
            </div>

            <div :class="{ hidden: !inEditingMode }" class="relative">
              <div class="absolute top-0 z-50 transform -translate-y-1/2 bg-gray-50 left-3 font-semibold text-sm rounded-full px-px">
                Hosting time
              </div>
              <vue-timepicker
                format="HH:mm"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getHostingDate() {
        this.pickedDate.setHours(this.pickedTime.HH != '' ? this.pickedTime.HH * 1 : 0, this.pickedTime.mm != '' ? this.pickedTime.mm * 1 : 0, 0);
