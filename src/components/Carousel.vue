  ...
  >
    ...
    <div class="w-full h-full z-40 pointer-events-none flex justify-between items-center absolute">
      ...
      >
        ...
        >
          <i class="ri-arrow-left-s-line mr-px text-sm mg:text-lg" aria-hidden="true" />
        </span>
      </div>
      <div
        :style="[dir == 1 ? transform : '']"
      ...
      >
        <span
          class="h-6 w-6 md:h-8 md:w-8 bg-gray-100 dark:bg-gray-900 shadow-lg flex items-center justify-center rounded-lg transform opacity-50 group-hover:-translate-x-2 group-hover:scale-125 group-hover:opacity-100 transition duration-300"
        >
          <i class="ri-arrow-right-s-line ml-px text-sm mg:text-lg" aria-hidden="true" />
        </span>
      </div>
    </div>

    <!-- slide container -->
    <transition :name="dir > 0 ? 'fade-left' : 'fade-right'" mode="in-out" appear>
      <div
    ...
    <div class="w-full h-full flex justify-center items-center">
      ...
      >
        ...
        <transition name="fade-y" mode="out-in" appear>
          <div
            v-if="!loading"
          ...
          >
            <span
              class="rounded-full bg-red-400 h-12 w-12 md:h-16 md:w-16 transform scale-75 group-focus:scale-100 transition duration-300 flex-none overflow-hidden"
            >
              <img :src="user?.img" alt />
            </span>
            <div
              class="flex flex-col justify-center items-start pl-4 w-full group-focus:pointer-events-auto pointer-events-none"
            >
              <h1
...
export default {
  ...
  computed: {
    currentImg() {
      ...
      if (!this.posters[this.posts[this.current]?.pid]) return "background-image: url(/img/icons/errorCover.jpg);";
