<template>
  <div class="px-10 py-4 h-16 w-full bg-gray-100 shadow-md flex items-center justify-start xl:rounded-b-4xl gap-4">
    ...
    <!-- //!actions -->
    <div v-if="getPrivLevel > 1" class="flex justify-center items-center gap-2 px-2 py-1 bg-gray-100 rounded-2xl shadow-sm">
      <h1 class="text-2xl font-bold select-none">Actions</h1>
      ...
      <!-- //!Create post button -->
      <div @click="goToCreate" class="flex justify-center items-center">
        <div class="flex justify-center items-center space-x-2 transform transition duration-300 bg-gray-200 bg-opacity-70 rounded-xl">
          <div class="flex-grow-0 flex justify-start items-center overflow-hidden group duration-300 cursor-pointer px-3 py-1.5 relative flex-shrink-0">
            <i
    ...
    <div>
      <div class="relative" v-if="getLoggedState">
        ...
        <div
          :class="{ 'h-0 opacity-20': !profileMenu, 'h-24 opacity-100': profileMenu }"
          ref="profileMenu"
        ...
        >
          <div class="px-4 py-2 space-y-2">
            ...
            <div @click="logOut" class="text-xl bg-red-400 rounded-lg px-8 py-1 w-full font-semibold btnTransform text-center cursor-pointer">
              <h1>Disconnect</h1>
            </div>
          </div>
        </div>
      ...
      <div v-else>
        <div @click="logIn()" class="cursor-pointer py-2 px-4 bg-red-100 rounded-xl btnRing" tabindex="-1">
          Sign In
        </div>
      </div>
    </div>
