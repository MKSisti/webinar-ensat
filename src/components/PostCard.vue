  ...
  >
    ...
    <div class="w-full transform group-hover:-translate-y-2 group-hover:scale-105 sm:group-hover:-translate-y-0 sm:group-hover:scale-100 transition duration-300 flex-shrink-0">
      ...
      <div class="h-32 w-full px-8 flex justify-start items-center">
        <div class="rounded-full h-16 w-16 pointer-events-auto transition duration-300 flex-shrink-0 overflow-hidden object-contain flex justify-center items-center">
          <img
            v-if="userData"
            :src="userData.img"
        ...
        <div class="flex flex-col justify-center items-start h-full px-3 w-full gap-y-2">
          ...
          >
            <h1
              v-if="userData"
              class="text-xl max-w-full truncate pr-36 flex-shrink-0 hover:underline -mt-2"
            >
              {{ userData?.userName }}
            </h1>
            <div
      ...
      <div class="px-12 sm:px-36 mt-4">
        <div class="bg-gray-100 py-1.5 px-2 rounded-2xl shadow-md flex justify-between items-center">
          <h1 class="text-sm sm:text-base font-bold">
            Hosting {{ hostedDate.date + ' at ' + hostedDate.time }}
          </h1>
          <div
            class="btnTransform cursor-pointer w-10 h-10 bg-red-400 flex-shrink-0 rounded-xl flex justify-center items-center"
            @click.prevent.stop
          >
            <i
              class="fa fa-bell text-base h-0 w-0 flex justify-center items-center"
              aria-hidden="true"
            />
          </div>
        </div>
