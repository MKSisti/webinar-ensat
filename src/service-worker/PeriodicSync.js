...
if (typeof ServiceWorkerRegistration.prototype.periodicSync == "undefined") {
  ...
  class PeriodicSyncManager {
    constructor() {
      ...
      window.periodicSyncManager.worker.addEventListener(
        "message",
    ...
    _onMessage(event) {
      ...
      if (event.data.type == "dispatchEvent") {
        ...
        document.dispatchEvent(
          window.periodicSyncManager.events[event.data.tag]
        );
    ...
    _bindBatteryEvents(battery) {
      battery.addEventListener(
        "chargingchange",
        this._onBatteryCargingChange.bind(this)
      );
    }
    ...
    _postBatteryInformation() {
      if (typeof navigator.getBattery != "undefined") {
        navigator.getBattery().then(function (battery) {
          ...
          window.periodicSyncManager.worker.postMessage({
    ...
    register(registerOptions) {
      ...
      return new Promise(function PeriodicSyncManagerRegistration(
        resolve,
        reject
      ) {
          ...
          if (event.data.type == "register") {
            if (event.data.tag === options.tag) {
              if (event.data.success == false) reject(event.data.error);
                ...
                window.periodicSyncManager.events[options.tag] =
                  new CustomEvent("sync-" + options.tag, registration);

                resolve(window.periodicSyncManager.syncs[options.tag]);
              }

              //event.target.removeEventListener(event.type, arguments.callee);
              window.periodicSyncManager.worker.removeEventListener(
                "message",
                onRegisterMessage
              );
            }
    ...
    _unregister(tagName) {
      return new Promise(function PeriodicSyncManagerUnRegistration(
        resolve,
        reject
      ) {
    ...
    getRegistration(tagName) {
      return new Promise(function PeriodicSyncManagerGetRegistration(
        resolve,
        reject
      ) {
    ...
    getRegistrations() {
      return new Promise(function PeriodicSyncManagerGetRegistrations(
        resolve,
        reject
      ) {
    ...
    permissionState() {
      return new Promise(function PeriodicSyncManagerPermissionState(
        resolve,
        reject
      ) {
        resolve(true);
